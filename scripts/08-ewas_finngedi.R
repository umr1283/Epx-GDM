...
output_directory <- here("outputs", "08-ewas_finngedi")
    ...
    pwalk(
      ...
      .f = function(.model, .form, .dmp) {
        ...
        ggsave(
          filename = file.path(
            output_directory, trait, 
            glue("{project_name}_EWAS_DMP_{trait}_{om_status}_{.model}.png")
          ),
          plot = p_all,
...
walk(traits, function(trait) {
  data_models <- map_df(c("mothers", "offsprings"), function(om_status) {
    data_models <- read_rds(file.path(output_directory, trait, glue("ewas_{trait}_{om_status}.rds")))
    ...
    gtsave(file.path(output_directory, trait, glue("{ project_name }_EWAS_summary.html")))
})
...
walk(traits, function(trait) {
  ...
  archive_name <- file.path(
    output_directory, 
    glue('{gsub("-", "", Sys.Date())}_{project_name}_EWAS_{trait}.zip')
  )
  zip(
