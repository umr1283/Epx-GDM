...
output_directory <- here("outputs", "10-ewas_finngedi_lmm")
  ...
  pwalk(
    ...
    .f = function(.model, .form, .term, .dmp, .form_o, .form_m) {
      ...
      ggsave(
        filename = file.path(
          output_directory, trait, 
          glue('{project_name}_EWAS_DMP_{trait}_{om_status}_{.model}_{gsub(":|Offspring", "", .term)}.png')
        ),
        plot = p_all,
...
walk(traits, function(trait) {
  ...
  archive_name <- file.path(
    output_directory,
    glue('{gsub("-", "", Sys.Date())}_{project_name}_EWAS_LMM_{trait}.zip')
  )
  zip(
