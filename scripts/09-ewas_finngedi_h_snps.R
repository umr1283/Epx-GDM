...
output_directory <- here("outputs", "09-ewas_finngedi_h_snps")
    ...
    pwalk(
      ...
      .f = function(.model, .form, .dmp, .term) {
        ...
        ggsave(
          filename = file.path(
            output_directory, trait, 
            glue('{project_name}_EWAS_DMP_SNPs_{trait}_{om_status}_{.model}_{gsub(":", "", .term)}.png')
          ),
          plot = p_all,
...
walk(traits, function(trait) {
  ...
  archive_name <- file.path(
    output_directory, 
    glue('{gsub("-", "", Sys.Date())}_{project_name}_EWAS_SNPs_HERITABILITY_{trait}.zip')
  )
  zip(
