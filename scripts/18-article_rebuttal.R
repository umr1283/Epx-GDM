...
output_directory <- here("outputs", "18-article_rebuttal")
...
theme_set(theme_light(base_family = "Tex Gyre Termes", base_size = 12)) 
theme_update(
...
res_om <- fread(here(
  "outputs", "16-ewas_finngedi_h_rebuttal", "GDM", 
  "EpxGDM_EWAS_DMP_GDM_mothers_offsprings_simple_cell_mumwtgain.csv.gz"
...
### Phenotypes Data ================================================================================
phenotype_matrix <- fread(file = file.path(data_directory, "EPIC", "EPIC_QC_phenotypes.csv"))[Project == "FinnGeDi"]
excel_pheno <- merge(
  x = setDT(read_xlsx(
    here("docs/FinnGedi_clinical_data_2018_10_05/FinnGeDi_selection_EpxGDM_2018_10_06_TO_TOBY.xlsx")
  )),
  y = setDT(read_xlsx(
    here("docs/FinnGedi_clinical_data_2018_10_05/FinnGeDi_selection_EpxGDM_SDscore_LGA_SGA_2021_02_10_TO_TOBY.xlsx")
  )),
  by = c("ID", "GDM", "BWSDSfi13")[1:2]
)[
  j = sex := factor(c("1" = "Male", "2" = "Female")[as.character(sex)], levels = c("Male", "Female"))
][ID %in% unique(phenotype_matrix$ID)][
  j = education := factor(
    x = education, 
    levels = c(1, 2, 3, 4),
    labels = c("1 = Basic", "2 = Secondary", "3 = Lower-level tertiary", "4 = Upper-level tertiary")
  )
][
  j = ses_4lk := factor(
    x = ses_4lk, 
    levels = c(1, 2, 3, 4),
    labels = c("1 = Highest", "2", "3", "4")
  )
][
  j = mgdm := fifelse(mgdm %in% 3, NA_real_, mgdm)
][
  j = c("mgdm", "mgdm2", "mfatherdm", "mmotherdm") := lapply(.SD, function(x) {
    factor(
      x = x, 
      levels = c(1, 2),
      labels = c("Yes", "No")
  ...
  .SDcols = c("mgdm", "mgdm2", "mfatherdm", "mmotherdm")
][
  j = primipara := factor(
      x = primipara, 
      levels = c(1, 0),
...
tbl_summary(
  ...
  by = "GDM", 
  include = c(
    "GDM", "mumage", "prebmi", "gestwk", "sex", "bwt", 
    "BWSDSfi13.y", "primipara", "previousdeliveries", "ModeOfDelivery",
    "LGA.90", "SGA.90", "mumwtgain", "mgdm", "mgdm2", "mfatherdm",
    "mmotherdm", "ses_4lk", "education"
  ), 
  statistic = list(
  ...
  label = list(
    ...
    bwt ~ "Birthweight (g)",
    BWSDSfi13.y ~ "Birthweight SD score",
    primipara ~ "Primipara (yes)",
    previousdeliveries ~ "Previous deliveries",
    ModeOfDelivery ~ "Mode of delivery (vaginal)",
    LGA.90 ~ "Large gestational age 90",
    SGA.90 ~ "Small gestational age 90",
    mumwtgain ~ "Maternal weight gain (kg)",
    mgdm ~ "Participant's mother GDM same pregnancy (yes)",
    mgdm2 ~ "Participant’s mother GDM any pregnancy (yes)",
    mfatherdm ~ "Mother's father diabetes (yes)",
    mmotherdm ~ "Mother's mother diabetes (yes)",
    ses_4lk ~ "Maternal socioeconomic status",
...
fwrite(
  x = fread(here(
    "outputs", "17-ewas_finngedi_rebuttal", "GDM", 
    "EpxGDM_EWAS_DMP_GDM_offsprings_simple_cell_mumwtgain.csv.gz"
  ))[pvalue < 0.05],
...
fwrite(
  x = fread(here(
    "outputs", "17-ewas_finngedi_rebuttal", "GDM", 
    "EpxGDM_EWAS_DMP_GDM_mothers_simple_cell_mumwtgain.csv.gz"
  ))[pvalue < 0.05],
...
beta_matrix[, (samples_cols) := lapply(.SD, logit2), .SDcols = samples_cols]
...
fmt_fdr_significant_results <- function(x) {
  x %>% 
  ...
  fmt_number(columns = vars(estimate_beta, std.error_beta), n_sigfig = 3, drop_trailing_zeros = FALSE) %>% 
  cols_merge_uncert(col_val = "estimate_beta", col_uncert = "std.error_beta") %>% 
  tab_footnote(
    footnote = md(paste(
      'Variable term or interaction term from the linear regression,',
      'where "Mothers" is the methylation of the mothers as M<sub>value</sub>'
    )), 
    locations = cells_column_labels("term")
  ) %>% 
  tab_footnote(
    footnote = md(
      "Linear regression coefficient plus/minus standard error on the M<sub>value</sub> or β<sub>value</sub> scale."
    ), 
    locations = cells_column_labels(c("estimate", "estimate_beta"))
  ) %>% 
  tab_footnote(
    footnote = md(
      "Linear regression p-value computed on the M<sub>value</sub> scale."
    ), 
  ...
  tab_footnote(
    footnote = md(paste(
      ...
      "with M<sub>value</sub> greater than 3 times the interquartile",
  ...
  cols_move(columns = "avgbvalue_meth", after = "estimate_beta") %>% 
...
fdr_significant_results <- local({
  ...
  dmp_beta <- rbindlist(lapply(
    ...
    FUN = function(icpg, .form, .form_m, .beta_o, .beta_m, .pheno) {
      ...
      lm_mother_resid <- .pheno %>%
        filter(grepl("m", Sample_ID)) %>%
        mutate(Mothers = ilogit2(as.matrix(.beta_m)[icpg, Sample_ID])) %>%
        do(
          add_residuals(
            data = .,
            model = lm(formula = update.formula(.form_m, "Mothers ~ ."), data = .)
      ...
      lm_pheno <- .pheno %>%
        ...
        mutate(MID = gsub("c", "m", Sample_ID)) %>% 
        left_join(y = lm_mother_resid, by = c("MID" = "Sample_ID")) %>% 
        mutate(Offsprings = ilogit2(Offsprings)) %>% 
        select(Sample_ID, all.vars(.form), Offsprings, Mothers) %>%
        drop_na()
  ...
  merge(
    x = merge(
      ...
      y = merge(x = dmp, y = dmp_beta, by = c("term", "CpG"), suffixes = c("", "_beta")), 
      by = c("CpG", "n")
    ),
    y = res_om[, c("CpG", "term", "fdr")],
    by = c("CpG", "term"),
    all.x = TRUE
  )[
    j = .(
...
fdr_significant_results[
  grepl("GDM", term, ignore.case = TRUE)
][order(CpG, term)][j = term := as.character(term)] %>% 
  gt() %>%
  fmt_scientific("fdr", drop_trailing_zeros = FALSE, decimals = 2, dec_mark = decimal_mark) %>% 
  cols_label(fdr = "FDR") %>% 
  cols_move(columns = "fdr", after = "p.value") %>% 
  fmt_fdr_significant_results() %>% 
  gtsave(filename = file.path(output_directory, "supplementary_table5.html"))


### Supplementary Table 5 (+ Mother term) ==========================================================
fdr_significant_results[
  j = term := factor(term, levels = c("GDM", "GDM &times; Mother", "Mother"))
][order(CpG, term)][j = term := as.character(term)][j = -c("fdr")] %>%
  gt() %>%
  fmt_fdr_significant_results() %>% 
  gtsave(filename = file.path(output_directory, "supplementary_table5_full.html"))
...
p_b <- ggplot(data = res_om[term == "GDM"]) +
  ...
  scale_shape_manual(
    values = 1:res_om[fdr < 0.05, uniqueN(CpG)],
    drop = FALSE, 
    guide = guide_legend(
      direction = "horizontal", 
      title = NULL, 
...
res_o <- rbind(
  fread(here(
    "outputs", "17-ewas_finngedi_rebuttal", "GDM", 
    "EpxGDM_EWAS_DMP_GDM_offsprings_simple_mumwtgain.csv.gz"
  ))[, type := "RAW"],
  fread(here(
    "outputs", "17-ewas_finngedi_rebuttal", "GDM", 
    "EpxGDM_EWAS_DMP_GDM_offsprings_simple_cell_mumwtgain.csv.gz"
...
res_m <- rbind(
  fread(here(
    "outputs", "17-ewas_finngedi_rebuttal", "GDM", 
    "EpxGDM_EWAS_DMP_GDM_mothers_simple_mumwtgain.csv.gz"
  ))[, type := "RAW"],
  fread(here(
    "outputs", "17-ewas_finngedi_rebuttal", "GDM", 
    "EpxGDM_EWAS_DMP_GDM_mothers_simple_cell_mumwtgain.csv.gz"
...
res_lmm <- fread(here(
  "outputs", "15-ewas_finngedi_lmm_rebuttal", "GDM", 
  "EpxGDM_EWAS_DMP_GDM_lmm_simple_cell_mumwtgain.csv.gz"
...
print(
  ggplot(data = gtex_data[like(Description, "^HIST1H3E$")]) + # H3C6
    aes(x = fct_reorder(Tissue, TPM), y = TPM, fill = group) +
    geom_col() +
    scale_x_discrete(labels = toupper, guide = guide_axis(angle = 45)) +
    scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
...
agg_tiff(
  filename = file.path(output_directory, "supplementary_figure8.tiff"), 
  width = 117, 
  height = 87.75, 
  units = "mm",
  res = 300,
...
agg_tiff(
  filename = file.path(output_directory, "supplementary_figure9.tiff"), 
  width = 117, 
  height = 87.75, 
  units = "mm",
  res = 300,
...
print(
  ggplot(data = gtex_data[like(Description, "^UBE3C$")]) +
    aes(x = fct_reorder(Tissue, TPM), y = TPM, fill = group) +
    geom_col() +
    scale_x_discrete(labels = toupper, guide = guide_axis(angle = 45)) +
    scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
    ...
    theme(
      ...
      axis.text = element_markdown(size = rel(0.6)),
      axis.text.x = element_markdown(size = rel(0.75)), 
      plot.margin = margin(5.5, 5.5, 5.5, 5.5 * 2, "pt")
    )
)
invisible(dev.off())
...
if (!Sys.getenv("USER") %in% c("root", "") && file.exists("~/.fex/id")) {
  local({
    ...
    archive_name <- file.path(
      ...
      paste(format(Sys.Date(), format = "%Y%m%d"), project_name, "rebuttal.zip", sep = "_")
    )
