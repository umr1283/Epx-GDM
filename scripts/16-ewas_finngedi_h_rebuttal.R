...
output_directory <- here("outputs", "16-ewas_finngedi_h_rebuttal")
      ...
      pwalk(
        ...
        .f = function(.model, .form, .dmp, .term) {
          ...
          ggsave(
            filename = file.path(
              output_directory, trait, 
              glue('{project_name}_EWAS_DMP_{trait}_{om_status}_{.model}_{gsub(":", "", .term)}_{gsub(" + ", "_", .model_type, fixed = TRUE)}.png')
            ),
            plot = p_all,
